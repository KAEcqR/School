...
null, '3tip_cw1');//wywołanie konstruktora klasy mysqli
...
function saveStudentToDb(string $firstname, string $lastname, int $group) : void {
    $conn = getConnection();
    if($conn==null) return;
    $sql = "INSERT INTO students (firstname, lastname, group_id)" . "VALUES ('{$firstname}', '{$lastname}', {$group})";
    echo $sql;
    $conn->query($sql);
    $conn->close();
}

function removeStudentFromDb(string $id) :void {
    $conn = getConnection();
    if($conn==null) return;
